...
test('4.10 a valid blog can be added ', async () => {
  ...
  const loginRes = await api.post('/api/login').send(testUser).expect(200)
  ...
  expect(resTitle).toContain('Uuden bloggaajan uudet tarinat')
})
...
test('4.13 deleting a blog', async () => {
  ...
  const blogsAtEnd = await api.get('/api/blogs').expect(200)
  console.log(
    'blogilistan pituus jälkeen poiston',
