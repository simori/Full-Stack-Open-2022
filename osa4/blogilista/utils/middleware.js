...
const errorHandler = (error, request, response, next) => {
  ...
  if (error.name === 'CastError') {
    ...
    return response.status(400).json({ error: 'blog not found! or other bad error happened!' })
  } else if (error.name === 'JsonWebTokenError') {
    return response.status(401).json({ error: 'unauthorized because token not provided or invalid!' })
  }
